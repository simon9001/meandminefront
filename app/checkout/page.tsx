...
import { ShoppingBag, MapPin, Tag, Loader2, ArrowLeft, CheckCircle, AlertCircle, BookmarkCheck, Plus } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  // Pre-fill form from the user's default saved address on first load
  useEffect(() => {
    ...
    setSaveAddress(false); // already saved — no need to re-save
      ...
      window.PaystackPop.setup({
        ...
        onSuccess: async (txn: { reference: string }) => {
          ...
          try {
            ...
            // Save address if the user asked and it's a new one (not a pre-saved address)
                    ...
                    {savedAddresses.map((addr) => (
                      ...
                      >
                        <span className="text-xs font-bold text-forest-900 flex items-center gap-1.5">
                          {addr.label}
                          {addr.is_default && (
                            <span className="text-[9px] bg-forest-100 text-forest-700 px-1.5 py-0.5 rounded-full font-semibold">
                              Default
                            </span>
                          )}
                        </span>
